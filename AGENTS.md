...
3. Tests (Vitest multi-project): unit + integration tests: npm test. Unit only: npm run test:unit (watch: test:unit:watch). Integration only: npm run test:integration (watch: test:integration:watch) after seed env (npm run docker:compose:up or npm run seed). Single test file: npx vitest run path/to/file.unit.test.ts.
...
9. Naming: files kebab-case; test files: _.unit.test.ts / _.integration.test.ts in `__tests__` folders. Env vars UPPER_SNAKE_CASE. Constants UPPER_SNAKE_CASE. Avoid default exports; prefer named.
