...
- An ACM certificate in `us-east-1` using [`terraform-aws-modules/acm/aws`](https://registry.terraform.io/modules/terraform-aws-modules/acm/aws/latest) with DNS validation records managed via Cloudflare for the `mattis.aasan.dev` domain.
- A CloudFront distribution with Origin Access Control (OAC) secured to the Lambda Function URL and Cloudflare DNS CNAME records.
...
| `parent_domain` | Cloudflare-managed zone (defaults to `aasan.dev`).                         |
...
5. Use the CloudFront domain or Cloudflare DNS record for application traffic, and run `npm run db:migrate` with the generated `database_url` output.
